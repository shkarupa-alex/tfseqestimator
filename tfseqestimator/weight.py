...
WEIGHTS_KEY = '_sequence_weights'


def make_sequence_weights(features, weight_column, sequence_logits, sequence_length):
    """Evaluate weight for sequence items loss: [user_weight] * sequence_mask / sequence_length

    Args:
      features: `dict` containing the input and sequence length information.
      weight_column: A string defining feature column representing weights.
...
def make_full_weights(features, weight_column, sequence_logits):
    """Evaluate weight for full sequence loss: user_weight
    ...
    with tf.variable_scope('weights'):
        with tf.name_scope('user_weights'):
            ...
            if weight_column is None:
                return tf.ones_like(sequence_logits)
